...
public class CalcBinomial {
	...
	CalcBinomial(double second, String javaScriptPath, String extension) {
	...
	protected List<Double> generateProbaBino() {
		...
		double 			p = sec / (HOUR* 3600.0);
		
		for (int i = 0 ; i <= 50 ; i++) {
			try {
				Object object = invocableEngine.invokeFunction("probBinomial", new Object[]{NB_CALL, p, i});
	...
	public List<Double> getBinoProba() {
		return probaBino;
	}
